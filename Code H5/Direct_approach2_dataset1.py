...
if __name__ is "__main__":
    ...
    settings['model'] = 'unet_GRL_approach1_2'
    ...
    for setting in obf_weights:
        ...
        for number in range(3):
            ...
            #train models
            folder = 'results/direct_approach2_dataset1/weight_{}_number_{}'.format(settings['loss_weight_obf'], number)
            ...
            comb.compile(optimizer=Adam(lr=settings['learning_rate_att']), loss = ['mean_absolute_error', 'binary_crossentropy', 'binary_crossentropy'], loss_weights=[obf_loss_weight, att_loss_weight, att_loss_weight])
            ...
            for epoch in tqdm(range(settings['epochs'])):
                ...
                for step in range(train_steps_per_epoch):
                    ...
                    metrics_public_obf = obfuscator.train_on_batch(x_batch_public, x_batch_public)
                    metrics_comb = comb.train_on_batch([x_batch_private, x_batch_public], [x_batch_private, y_batch_label_private, y_batch_label_public])
                    
                    metrics['train_public_loss'][epoch] += metrics_public_obf/train_steps_per_epoch
                    metrics['train_private_loss'][epoch] += metrics_comb[1]/train_steps_per_epoch
                    
                    metrics['att_train_public_loss'][epoch] += metrics_comb[3]/train_steps_per_epoch
                    metrics['att_train_private_loss'][epoch] += metrics_comb[2]/train_steps_per_epoch
                ...
                for step in range(val_steps_per_epoch):
                    ...
                    metrics_public_obf = obfuscator.test_on_batch(x_batch_public, x_batch_public)
                    metrics_comb = comb.test_on_batch([x_batch_private, x_batch_public], [y_batch_image_private, y_batch_label_private, y_batch_label_public])
                    
                    metrics['val_public_loss'][epoch] += metrics_public_obf/val_steps_per_epoch
                    metrics['val_private_loss'][epoch] += metrics_comb[1]/val_steps_per_epoch
                    
                    metrics['att_val_public_loss'][epoch] += metrics_comb[3]/val_steps_per_epoch
                    metrics['att_val_private_loss'][epoch] += metrics_comb[2]/val_steps_per_epoch
