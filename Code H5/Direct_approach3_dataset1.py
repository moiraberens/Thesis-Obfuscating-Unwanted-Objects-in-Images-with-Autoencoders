...
if __name__ is "__main__":
    ...
    for setting in obf_weights:
        ...
        for number in range(3):
            ...
            folder = 'results/direct_approach3_dataset1/weight_{}_number_{}'.format(settings['loss_weight_obf'], number)
            ...
            comb.compile(optimizer=Adam(lr=settings['learning_rate_att']), loss = ['mean_absolute_error', 'binary_crossentropy'], loss_weights=[obf_loss_weight, att_loss_weight])
            ...
            metrics['att_train'] = np.zeros(settings['epochs'])
            ...
            for epoch in tqdm(range(settings['epochs'])):
                ...
                for step in range(train_steps_per_epoch):
                    ...
                    #train comb
                    metrics_public = comb.train_on_batch(x_batch_public, [x_batch_public, y_batch_label_public])
                    metrics_private = comb.train_on_batch(x_batch_private, [x_batch_private, y_batch_label_private])
                    
                    metrics['train_public_loss'][epoch] += metrics_public[1]/train_steps_per_epoch
                    metrics['train_private_loss'][epoch] += metrics_private[1]/train_steps_per_epoch
                    
                    metrics['att_train_public_loss'][epoch] += metrics_public[2]/train_steps_per_epoch
                    metrics['att_train_private_loss'][epoch] += metrics_private[2]/train_steps_per_epoch
                ...
                for step in range(val_steps_per_epoch):
                    ...
                    metrics_public = comb.test_on_batch(x_batch_public, [x_batch_public, y_batch_label_public])
                    metrics_private = comb.test_on_batch(x_batch_private, [y_batch_image_private, y_batch_label_private])
                    
                    metrics['val_public_loss'][epoch] += metrics_public[1]/val_steps_per_epoch
                    metrics['val_private_loss'][epoch] += metrics_private[1]/val_steps_per_epoch
                    
                    metrics['att_val_public_loss'][epoch] += metrics_public[2]/val_steps_per_epoch
                    metrics['att_val_private_loss'][epoch] += metrics_private[2]/val_steps_per_epoch
