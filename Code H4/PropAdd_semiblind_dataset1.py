...
def make_private_mask(position_list):
    private_mask = np.zeros((settings['batch_size'], settings['img_size'], settings['img_size'],1))
    private_mask_part = np.ones((settings['obj_size'], settings['obj_size'],1))
    for index in range(0, len(private_mask)):
        private_mask[index, position_list[index][0]:position_list[index][0] + settings['obj_size'], position_list[index][1]:position_list[index][1]+settings['obj_size']] = private_mask_part
    return private_mask
...
if __name__ is "__main__":
    ...
    settings['whole'] = False            # True, False
    ...
    for iteration in range(3):
        ...
        folder = 'results/propAdd_blind_dataset1/{}'.format(iteration)
        
        obfuscator, obfuscator_part = get_model.load_model(settings)
        if not os.path.exists(folder):
            os.makedirs(folder)
        plot_model(obfuscator, to_file=folder + '\model.png', show_shapes=True)
        obfuscator.compile(optimizer = Adam(lr=settings['learning_rate']), loss = settings['loss'])
        obfuscator_part.compile(optimizer = Adam(lr=settings['learning_rate']), loss = [settings['loss'], settings['loss']])
        ...
        public_mask = np.zeros((settings['batch_size'], settings['img_size'], settings['img_size'],1))
        ...
        for epoch in tqdm(range(settings['epochs'])):
            ...
            for step in range(train_steps_per_epoch):
                ...
                private_mask = make_private_mask(position_list)
                ...
                if settings['same_batch'] == True:
                    ...
                    m_batch_1 = np.concatenate((public_mask[:settings['half_batch']], private_mask[:settings['half_batch']]))
                    ...
                    m_batch_2 = np.concatenate((public_mask[-settings['half_batch']:], private_mask[-settings['half_batch']:]))
                    y_batch_2 = np.concatenate((x_batch_public[-settings['half_batch']:], y_batch_image_private[-settings['half_batch']:]))
                    obfuscator_part.train_on_batch(x_batch_1, [m_batch_1, y_batch_1])
                    obfuscator_part.train_on_batch(x_batch_2, [m_batch_2, y_batch_2])
                    ...
                    obfuscator_part.train_on_batch(x_batch_public, [public_mask, x_batch_public])
                    obfuscator_part.train_on_batch(x_batch_private, [private_mask, y_batch_image_private])
                    metrics['train_public_loss'][epoch] += obfuscator.test_on_batch(x_batch_public, x_batch_public)/train_steps_per_epoch
                    metrics['train_private_loss'][epoch] += obfuscator.test_on_batch(x_batch_private, y_batch_image_private)/train_steps_per_epoch
                    private_pred = obfuscator.predict(x_batch_private)
                    metrics['train_obj_loss'][epoch] += calculate_obj_loss(settings, private_pred, y_batch_image_private, position_list)/train_steps_per_epoch
